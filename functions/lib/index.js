...
// OWNER VERIFICATION
// ============================================
async function verifyOwner(context) {
    if (!context.auth)
...
exports.transferAsh = functions.https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (t) => {
        ...
        t.create(db.collection('users').doc(senderUid).collection('transactions').doc(), {
            type: 'transfer_sent',
            amount: -amount,
            fee: GAS_FEE,
        ...
        t.create(db.collection('users').doc(recipientUid).collection('transactions').doc(), {
...
exports.ownerEditBalance = functions.https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (t) => {
        ...
        t.create(userRef.collection('balanceAudit').doc(), {
            type: 'owner_edit',
            oldBalance,
            newBalance,
            reason,
